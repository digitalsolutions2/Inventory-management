...
export async function POST(
  ...
  try {
    ...
    // Deduct from source in transaction with atomic guards
    const updated = await prisma.$transaction(async (tx) => {
      for (const line of transfer.lines) {
        // Atomically deduct with guard against going negative
        ...
        if (result.count === 0) {
          throw new Error(
            `Insufficient stock for ${line.item.name} (concurrent modification)`
          );
        }
    ...
    console.error("POST /api/transfers/[id]/fulfill error:", e);
    const message =
      e instanceof Error && e.message.includes("Insufficient stock")
        ? e.message
        : "Failed to fulfill transfer";
