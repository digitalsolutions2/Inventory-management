...
export async function POST(
  ...
  const { lines, notes } = body as {
    lines: { id: string; receivedQty: number; notes?: string }[];
    notes?: string;
  };
  ...
  for (const line of lines) {
    ...
    if (!trfLine) return apiError(`Transfer line ${line.id} not found`);
    if (line.receivedQty < 0)
      return apiError("Received quantity cannot be negative");
  ...
  const updated = await prisma.$transaction(async (tx) => {
    for (const line of lines) {
      ...
      if (existingInv) {
        ...
        // Get avg cost from source or item
