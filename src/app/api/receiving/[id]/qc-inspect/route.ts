...
export async function POST(
  ...
  const { qcResult, lines, notes } = body as {
    ...
    lines: { id: string; acceptedQty: number; rejectedQty: number; notes?: string }[];
    notes?: string;
  };

  if (!qcResult || !["ACCEPTED", "PARTIAL", "REJECTED"].includes(qcResult)) {
    return apiError("Valid QC result is required (ACCEPTED, PARTIAL, REJECTED)");
  }
  ...
  if (receiving.status !== "PROC_VERIFIED") {
    return apiError("Receiving must be in Procurement Verified status for QC inspection");
  }
  ...
  const newStatus = qcResult === "REJECTED" ? "QC_REJECTED" : "QC_APPROVED";
