...
export async function POST(
  ...
  try {
    ...
    if (receiving.status !== "QC_APPROVED") {
      return apiError(
        "Receiving must be QC Approved for warehouse receiving"
      );
    }
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      // 3. Update PurchaseOrderLine receivedQty (cap at ordered quantity)
      for (const recLine of receiving.lines) {
        ...
        if (poLine) {
          const newReceivedQty = Math.min(
            poLine.receivedQty + recLine.acceptedQty,
            poLine.quantity
          );
    ...
    console.error(
      "POST /api/receiving/[id]/warehouse-receive error:",
      e
    );
    return apiError("Failed to process warehouse receiving", 500);
  }
}
