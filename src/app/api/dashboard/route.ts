...
export async function GET() {
  ...
  const [
    ...
    // Low stock items (inventory below reorder point)
    prisma.inventoryItem.findMany({
      where: { tenantId },
      include: {
        item: { select: { id: true, code: true, name: true, uom: true, reorderPoint: true, minStock: true } },
    ...
    prisma.purchaseOrder.findMany({
      where: {
        tenantId,
        status: { in: ["APPROVED", "SENT", "PARTIALLY_RECEIVED", "RECEIVED"] },
  ...
  const lowStock = lowStockItems.filter(
    (inv) => inv.quantity <= inv.item.reorderPoint && inv.item.reorderPoint > 0
  );
